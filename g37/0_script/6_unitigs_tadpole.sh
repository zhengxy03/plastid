...
# Run
#----------------------------#
log_warn 0_script/6_unitigs_tadpole.sh
...
parallel --no-run-if-empty --linebuffer -k -j 1 "
    ...
    echo >&2 '==> 6_unitigs_tadpole/MRX{1}P{2}'
    if [ -e 6_unitigs_tadpole/MRX{1}P{2}/unitigs.fasta ]; then
        echo >&2 '    unitigs.fasta already presents'
        exit;
    fi

    mkdir -p 6_unitigs_tadpole/MRX{1}P{2}
    cd 6_unitigs_tadpole/MRX{1}P{2}
